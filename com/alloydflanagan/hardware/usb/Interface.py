...
class USBInterface(object):
    ...
    def __init__(self, intfc):
        ...
        if self.intfc.bDescriptorType != 4:
            raise InvalidArgumentType(
                    'Was expecting descriptor type 4, got {}'.format(
                        self.bDescriptorType))
        self.endpoints = [USBEndpoint(x) for x in intfc]
        #TODO: find the string descriptor and get a description of this
        #interface
    ...
    def dump(self):
        val = ("Interface: #{}, class {}, subclass {}, protocol {}, "
               "#endpoints: {}").format(
            self.bInterfaceNumber, self.bInterfaceClass,
            self.bInterfaceSubClass, self.bInterfaceProtocol,
            self.bNumEndpoints)
...
#Index of String Descriptor Describing this interface
    ...
    def __unicode__(self):
        return 'intf[{}]: {}({})'.format(self.bInterfaceNumber,
...
if __name__ == '__main__':
    ...
    for d in devs:
        for c in d:
            for i in c:
                #verify that USBInterface objects expose attributes of
                #Interface
                for attr in ['alternate_index', 'bAlternateSetting',
                             'bDescriptorType', 'bInterfaceClass',
                             'bInterfaceNumber', 'bInterfaceProtocol',
                             'bInterfaceSubClass', 'bLength', 'bNumEndpoints',
                             'configuration', 'iInterface', 'index']:
